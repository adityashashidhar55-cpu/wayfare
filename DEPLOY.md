...
3. Render finds `render.yaml` and asks for ONE value, `DATABASE_URL`: paste
   the string from step 1. Everything else is set or generated by Render
   (r34). The database name at the end of the string does not matter - the
   app creates it if it is missing.

4. Click **Apply**. The first build takes 5–8 minutes.
5. When it is live, Render gives you a URL like
   `https://wayfare.onrender.com`. The app picks that URL up by itself; set
   `APP_URL` only when you add a custom domain.
